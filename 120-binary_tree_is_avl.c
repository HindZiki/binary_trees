 ...
 * @tree: node point to the tree to check.
 * @high: node point to the higher node selected
 * @lower: node point to the lower node selected.
 * Return: 1 if tree is AVL 0 if not.
 */
int bal_avl(const binary_tree_t *tree, int lower, int high)
{
	size_t height_l, height_r, balanc;
	...
	if (tree != NULL)
	{
		...
		balanc = height_l > height_r ? height_l - height_r : height_r - height_l;
		if (balanc > 1)
		{
			return (0);
		}
